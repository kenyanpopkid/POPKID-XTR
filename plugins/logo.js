...
cmd({
    ...
    try {
        ...
        if (!effectData) {
            await reply(`❌ ${toTinyCaps('Invalid effect! Please choose from the available effects.')}`);
            await conn.sendMessage(from, { react: { text: '❌', key: mek.key } });
            return;
        }
        ...
        if (!apiResponse || !apiResponse.result || !apiResponse.result.download_url) {
            await reply(`❌ ${toTinyCaps(`Failed to generate logo with ${effectData.desc}. API may be down.`)}`);
            await conn.sendMessage(from, { react: { text: '❌', key: mek.key } });
            return;
        }
