...
`;
        ...
        const allEffects = [
            ...
            { name: 'typography': `https://api-pink-venom.vercel.app/api/logo?url=https://en.ephoto360.com/create-typography-status-online-with-impressive-leaves-357.html&name=${encodeURIComponent(text)}`,
            'bear': `https://api-pink-venom.vercel.app/api/logo?url=https://en.ephoto360.com/free-bear-logo-maker-online-673.html&name=${encodeURIComponent(text)}`
        };
...
`;
        ...
        const buttonsMessage = buttonManager.createButtonsMessage({
            imageUrl: LIST_IMAGE,
            caption,
            footer: toTinyCaps('> powered by popkid xtr'),
            buttons,
            contextInfo: {
        ...
        await client.sendMessage(from, {
            image: imageBuffer,
            caption: `> *${toTinyCaps('powered by popkid xtr')}*`,
            quoted
        });
...
textEffects.forEach(({ pattern, desc, react }) => {
    cmd({
        ...
        try {
            ...
            // FIX: Use a more stable and unique session ID based on sender and message key
            const sessionId = `${mek.sender}-${mek.key.id}`;
