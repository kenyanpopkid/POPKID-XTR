...
cmd({
    ...
    try {
        ...
        if (!args || args.length === 0) {
            const effectList = Object.keys(allEffects).map(key => `*${toTinyCaps(key)}*`).join(' | ');
            await reply(`❌ ${toTinyCaps('Please provide text and an effect for the logo')}.\n\n${toTinyCaps('Available effects')}: ${effectList}\n\n${toTinyCaps('Example')}: .logo dragonball popkid`);
        ...
        // If no text is provided after the effect, reply with an error
        if (!text) {
            await reply(`❌ ${toTinyCaps('Please provide a name for the logo')}. Example: .logo ${effectName} Empire`);
            await cmd.sendMessage(from, { react: { text: '❌', key: mek.key } });
            return;
        }
        ...
        // Fetch the logo image URL from the API
        ...
        // Get the image buffer from the download URL
        const imageBuffer = await getBuffer(apiResponse.result.download_url);
        
        // Define the status message based on the response
        const status = imageBuffer ? '✅ Processed Successfully' : '❌ Failed to process';

        // Get bot and owner info from config for the stylish panel
        ...
        // Create the stylish output panel, matching the ping command's format
        const output = `
┏━━━━━━━━━━━━━━━━━━━━━━┓
